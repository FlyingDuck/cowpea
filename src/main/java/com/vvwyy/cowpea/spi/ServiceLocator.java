...
import com.vvwyy.cowpea.Builder;
import com.vvwyy.cowpea.util.ClassLoading;
...
public final class ServiceLocator implements ServiceProvider<Service> {
    ...
    public <T extends Service> T getService(Class<T> serviceType) {
        if (serviceType.isAnnotationPresent(PluralService.class)) {
            throw new IllegalArgumentException(serviceType.getName() + " is marked as PluralService");
        }
        final Collection<T> registeredServices = getServicesOfType(serviceType);
        if (registeredServices.size() > 1) {
            throw new AssertionError("The non-PluralService type"
                    + serviceType.getName()
    ...
    public <T extends Service> boolean knowsServiceFor(Class<T> serviceClass) {
    ...
    public void startAllServices() {
        ...
        try {
            ...
            while (!unstarted.isEmpty()) {
                boolean startSomething = false; // Use to judge whether has cyclic dependency in Service set
                for (Iterator<Service> it = unstarted.iterator(); ((Iterator) it).hasNext(); ) {
                    Service s = it.next();
    ...
    public void stopAllServices() throws Exception {
        ...
        try {
            ...
            while (!running.isEmpty()) {
                boolean stoppedSomething = false; // Use to judge whether has cyclic dependency in Service set
